<CaseStudy>
  <CaseStudy.Header>확장 불가능한 Flat 컴포넌트 구조의 한계 극복</CaseStudy.Header>
  <CaseStudy.Body>
...
/>`}
    ...
    <CaseStudy.Section title="해결 과정 (Compound Pattern)" dotColor="bg-blue-400">
      컴포넌트의 제어권을 부모에게 넘겨주는 **Compound Component** 패턴을 도입하여 구조를 전면 리팩터링했습니다.
      `CaseStudy`를 `Header`, `Section`, `Code` 등의 하위 컴포넌트로 쪼개 이제 필요한 블록만 조립하여 사용할 수 있어 어떤 형태의 콘텐츠도 유연하게 대응할 수 있게 되었습니다.
...
<CaseStudy>
  <CaseStudy.Header>뒤로가기 시 스크롤 위치 소실 문제 해결</CaseStudy.Header>
  <CaseStudy.Body>
    <CaseStudy.Section title="문제 상황" dotColor="bg-red-400">
